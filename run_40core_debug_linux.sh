...
# Focused Linux debug workflow for the 40-thread HPX case:
# 1) Ensure vcpkg exists and is bootstrapped
# 2) Configure + build test_hpx
# 3) Create/use Python venv and install requirements
# 4) Run only the 40-thread case with HPX binding diagnostics enabled
...
log "Running the 40-thread HPX debug case with binding diagnostics"
# This intentionally runs the debug-only 40-thread mode and prints the HPX binding info.
./.venv/bin/python ./plot_thread_size_surface.py --exe "${EXE_PATH}" --debug-40 --print-bind "$@"

log "Done. Binding diagnostics are enabled for the 40-thread run."
